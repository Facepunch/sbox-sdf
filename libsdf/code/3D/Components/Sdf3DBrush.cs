...
[Title( "SDF 3D Brush" )]
public abstract class Sdf3DBrushComponent : Component, Component.ExecuteInEditor
{
	...
	public Modification<Sdf3DVolume, ISdf3D> NextModification
	{
		get
		{
			...
			var sdf = OnBuildSdf();
			var world = Components.GetInAncestors<Sdf3DWorldComponent>();

			var modifiers = Components.GetAll<Sdf3DModifierComponent>();
	...
	internal void InvalidateWorld()
	{
		...
		Components.GetInAncestors<Sdf3DWorldComponent>()?.InvalidateBrush( this );
	}
	...
	protected void Transform_Changed()
	{
		foreach ( var brush in Components.GetAll<Sdf3DBrushComponent>() )
		{
...
public abstract class Sdf3DModifierComponent : Component, Component.ExecuteInEditor
{
	public abstract ISdf3D Apply( ISdf3D sdf );

	private void InvalidateWorld()
	{
		Components.Get<Sdf3DBrushComponent>()?.InvalidateWorld();
	}
